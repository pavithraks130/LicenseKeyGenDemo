...
namespace LicenseKey
{
    ...
    public class GenerateKey
    {
        ...
        private int maxTokens; // the maximum number of tokens that was set
        private readonly Randomm rnd; // out random number class
        private string strLicensekey; // the final license key that was made. 
        private Tokens[] tokens; // the list of tokens that can be used in the lciense string. 
        ...
        public GenerateKey()
        {
            ...
            UseBase10 = false; // default to use base 16
            UseBytes = false; // default to use bits
        ...
        public Checksum.ChecksumType ChecksumAlgorithm { get; set; }
        ...
        public void CreateKey()
        {
            ...
            for (lop = 0; lop < maxTokens; lop++)
                try
                {
                    tokens[lop].datatype.UseBase10 = UseBase10;
                    tokens[lop].datatype.UseBytes = UseBytes;
                    tokens[lop].datatype.CheckToken(LicenseTemplate, tokens[lop].InitialValue,
                        tokens[lop].CharacterToken);
                }
                catch (Exception)
                {
                    throw;
        ...
        private static int GetChecksumToklength(string lickey)
        {
            int i;
            int cnt;
            int slen;
        ...
        /// <returns></returns>
        private bool CheckInputLegalBit(string lictemp, string lickey)
        {
            var answer = true;
            int remain;
            int tokenCnt;
            ...
            for (i = 0; i < slen; i++)
            {
                ...
                if (tokTem == 'c')
                {
                    ...
                    var toklen = GetChecksumToklength(lictemp);
                ...
                chkTemString = chkTemString + tokTem;
                if (remain == 0)
        ...
        private string DisassembleKeyBytes(string token)
        {
            ...
            localLicenseTemplate = LicenseTemplate;
            localLicensekey = strLicensekey;
            ...
            for (i = 0; i < slen; i++)
            {
                ...
                if (tokTem == token[0])
                {
                    LicTokenvalue = LicTokenvalue + tokLic;
                    TemTokenvalue = TemTokenvalue + tokTem;
                }
            }
            ...
            for (i = 0, j = 0; i < sLenTem; i++)
            {
                ...
                if (tokTem == token[0])
                {
                    ...
                    if (UseBase10)
                        ...
                        itokLic = int.Parse(result, NumberStyles.AllowHexSpecifier);
